...
%% Nat Comm Fig2 (Size Spectral Plots for Linear, LGP, GSC, and LGP + GSC)

figure('position',[400 400 800 800]);
cc=get(gca,'colororder');
orgfit=[1e-10 1e1];

I=find(tte(2,1:niter2)>=.0999 & tte(2,1:niter2)<=.1001);
...
[P,S2]=polyfit(log10(orgmass),log10(mass_dens(:,I(2))),1);
...
I=find(tte(2,(niter2+1):(2*niter2))>=.0999 & tte(2,(niter2+1):(2*niter2))<=.1001)+niter2;
...
[P,S2]=polyfit(log10(orgmass(1:end-2)),log10(mass_dens(1:end-2,I(2))),1);
...
I=find(tte(2,(2*niter2+1):(3*niter2))>=.0999 & tte(2,(2*niter2+1):(3*niter2))<=.1001)+2*niter2;
...
[P,S2]=polyfit(log10(orgmass(1:end-1)),log10(mass_dens(1:end-1,I(2))),1);
massfit1=10.^(P(2)).*orgfit.^P(1);
...
I=find(tte(2,(3*niter2+1):(4*niter2))>=.0999 & tte(2,(3*niter2+1):(4*niter2))<=.1001)+3*niter2;
...
%% NatCom Fig3 (Biomass and k versus theoretical for size structured ecosystems)
...
leg=legend('base','LGP','GSC','LGP+GSC');
...
%% Nat Comm Figure 4 (Bar Plot with biomass and k for Linear, LGP, GSC, and Combined
...
%% Nat Comm Fig5 (Bar Plot Comparison with Palmyra)

I=find(ttem(:,2)>.0999 & ttem(:,2)<.11)+niter2;

scale=50/80;

xx=mass_dens(2:5,I(3)); xx=xx./xx(1);
...
%%
imass=[1e-1 1e0 1e1 1e2];
%xx=mass_dens(2:5,1102)./.12.*1000.^2; %convert to kg per km2
[P,S]=polyfit(log10(orgmass(1:end-2)),log10(mass_dens(1:end-2,1102)),1);
xx(1:3)=10.^P(2).*imass(1:3).^P(1);
[P,S]=polyfit(log10(orgmass),log10(mass_dens(:,1102)),1);
xx(4)=10.^P(2).*imass(4).^P(1);
