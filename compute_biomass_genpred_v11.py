...
def main():
    ...
    #orgmass=np.array([1e-8, 1e-5, 1e-3, 1e-1, 1e2, 4800])
    ...
    #create output file
    outfile = '/Data/cobialab/conserveandclimate/model_data/tte_biomass_genpred_final_' + dt.datetime.strftime(dt.datetime.now(),'%Y%m%d') + '.nc'
    ...
    for ii in range(0,niter):
        ...
        #tmax=np.random.randint(10,500,size=1)*.01
        #tte=np.random.randint(1,tmax*100,size=ncat)*.001
        tte=np.random.randint(1,200,size=1)*.001*np.ones(ncat)
        tte[0]=0.0
        ...
        #distribute energy from each source across all consumers
        for jj in range(0,ncat):
            if(np.sum(tflow[jj,:])>0):
                tflow[jj,:]=tflow[jj,:]/np.sum(tflow[jj,:])
        
        #set primary production
        tflow[0,0]=1
        #distribute energy and account for trophic transfer (rows jj are prey, columns kk are consumers)
        for jj in range(0,ncat):
            for kk in range(1,ncat):
                if tflow[jj,kk]>0:
                    tflow[jj,kk]=tflow[jj,kk]*np.sum(tflow[:,jj])*tte[kk]
...
def compute_sat_biomass(sst1,gpp1,par1,tweb,endo1,orgmass1,tclass1):
    ...
    Po_p=1.32e9; E_p=0.32; beta_p=1.036; PAR=.84;
